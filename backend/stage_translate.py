...
def _looks_like_peft_adapter_dir(model_dir: Path) -> bool:
    return model_dir.is_dir() and (model_dir / "adapter_config.json").exists()
...
class StageTranslator:
    ...
    def _load_tokenizer_if_available(self) -> None:
        if self._tokenizer is not None or AutoTokenizer is None:
            return
        model_dir = _pick_best_model_dir(TAMIL_TO_THENI_MODEL_ROOT)
...
""".strip()
        return self._cleanup_tamil(
            self.core.generate_text(system_prompt, user_prompt, temperature=TRANSLATION_TEMPERATURE, max_output_tokens=900)
        )

    def _retry_translate_chunk(self, english_text: str) -> str:
        system_prompt = (
            "Translate the given English text into Tamil only. Do not keep it in English. "
            "Do not explain anything."
        )
        return self._cleanup_tamil(
            self.core.generate_text(system_prompt, english_text, temperature=0.0, max_output_tokens=900)
        )
...
""".strip()
        refined = self._cleanup_tamil(
            self.core.generate_text(system_prompt, user_prompt, temperature=0.08, max_output_tokens=900)
        )
    ...
    def english_to_tamil_with_meta(self, english_text: str, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not english_text:
            return {"tamil_text": "", "chunks": 0, "tone": "neutral", "answer_length": "short", "retry_count": 0}
    ...
    def tamil_to_thenitamil(self, tamil_text: str) -> str:
        ...
        if self._ensure_tamil_to_theni_model():
            try:
                ...
                self._clear_memory()
        system_prompt = (
            "Convert the given standard Tamil into natural, colloquial Theni-style Tamil. "
            "Keep the meaning the same and return only Tamil text."
        )
        return self._cleanup_tamil(
    ...
    def _generate(self, text: str, model: Any) -> str:
        ...
        encoded = self._tokenizer(text, return_tensors="pt", truncation=True, max_length=DIALECT_MODEL_MAX_LENGTH)
        output_ids = model.generate(**encoded, max_length=DIALECT_MODEL_MAX_LENGTH, num_beams=DIALECT_MODEL_NUM_BEAMS)
