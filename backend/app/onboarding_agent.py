...
except Exception as exc:  # pragma: no cover - exercised via import path verification
    LEGACY_ONBOARDING_AVAILABLE = False
    LEGACY_ONBOARDING_IMPORT_ERROR = str(exc)
    AIMessage = AnyMessage = HumanMessage = object  # type: ignore[assignment]
    OpenAIEmbeddings = MemorySaver = StateGraph = None  # type: ignore[assignment]
    add_messages = None  # type: ignore[assignment]
    END = "__END__"  # type: ignore[assignment]
    Chroma = TextLoader = RecursiveCharacterTextSplitter = None  # type: ignore[assignment]
    logger.warning(
        "Deprecated onboarding backend is unavailable because optional legacy dependencies failed to import: %s",
        exc,
    )
...
def _legacy_unavailable_error() -> HTTPException:
    detail = "Deprecated backend onboarding is unavailable. Mobile local-first onboarding is the primary path."
    if LEGACY_ONBOARDING_IMPORT_ERROR:
        detail = f"{detail} Optional legacy dependency failure: {LEGACY_ONBOARDING_IMPORT_ERROR}"
...
if not LEGACY_ONBOARDING_AVAILABLE:

    @router.post("/chat")
    async def onboarding_chat_unavailable(request: ChatRequest):
        logger.warning(
            "Deprecated onboarding route requested for user_id=%s but legacy extras are unavailable: %s",
            request.user_id,
            LEGACY_ONBOARDING_IMPORT_ERROR or "unknown import error",
        )
        raise _legacy_unavailable_error()
    ...
    def initialize_company_rag():
        try:
            ...
            if not os.path.exists(kb_file):
                logger.warning("Deprecated onboarding company_knowledge.json not found. Legacy RAG disabled.")
                return None
            ...
            logger.info("Deprecated onboarding RAG initialized with %s chunks from %s", len(splits), kb_file)
            return vectorstore.as_retriever(search_kwargs={"k": 3})
        except Exception as exc:
            logger.warning("Deprecated onboarding RAG init failed: %s", exc)
            return None
    ...
    def chat_node(state: AgentState):
        ...
        ai_question_count = sum(
            1 for m in messages if isinstance(m, AIMessage) and "DONE" not in getattr(m, "content", "").upper()
        )
        ...
        try:
            ...
            logger.warning("Deprecated onboarding could not read knowledge.json: %s", exc)
            flow_script = ""
        ...
        if messages and company_retriever:
            ...
            if last_human_msgs:
                ...
                try:
                    ...
                    logger.warning("Deprecated onboarding RAG retrieval error: %s", exc)
    ...
    *** QUESTION COUNTER: You have asked {ai_question_count} questions so far. ***
    ...
    """
        ...
        return {
            "messages": [
                AIMessage(
                    content=bot_tanglish,
                    additional_kwargs={"speak_tamil": bot_tamil, "options": bot_options},
                )
            ]
        }
    ...
    """
        ...
        client = OpenAI()
        conversation_msgs = [m for m in messages if "DONE" not in getattr(m, "content", "").upper()]
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
        ...
        try:
            ...
            logger.info("Deprecated onboarding profile saved to ChromaDB for user: %s", user_name)
        except Exception as exc:
            logger.warning("Deprecated onboarding ChromaDB save error: %s", exc)
