...
class DBJobQueue:
    def __init__(
        ...
        *,
        ...
        self.register(
            "web_post_turn_distillation",
            _handle_web_post_turn_distillation,
        )
        self.register(
            "web_memory_embedding_backfill",
            _handle_web_memory_embedding_backfill,
        )
        self.register(
            "global_qa_embedding_backfill",
            _handle_global_qa_embedding_backfill,
        )
        self.register(
            "web_knowledge_ingest",
            _handle_web_knowledge_ingest,
        )
        self.register(
            "web_embedding_backfill",
            _handle_web_embedding_backfill,
        )
        self.register(
            "web_triplet_extract",
            _handle_web_triplet_extract,
        )
        self.register(
            "web_hierarchy_build",
            _handle_web_hierarchy_build,
        )
    ...
    def enqueue(
        ...
        *,
        ...
        job = Job(
            ...
            run_at=utc_now()
            + timedelta(
                seconds=max(0, int(run_after_seconds))
            ),
            started_at=None,
            finished_at=None,
            created_at=utc_now(),
    ...
    def _claim_next_job(
        ...
        if self.strict_fifo and self.stale_running_seconds is not None:
            ...
            running_statement = select(Job.id).where(
                Job.status == "running"
            )
        ...
        statement = (
            ...
            # FIX: Only select jobs whose scheduled time has arrived.
            # Future jobs must not block other runnable jobs.
        ...
        if (
            candidate is None
            or (
                candidate_run_at is not None
                and candidate_run_at > now
            )
        ):
            return None
        ...
        claim_result = session.exec(
            claim_statement.execution_options(
    ...
    def _recover_stale_jobs(
        ...
        cutoff = now - timedelta(
            seconds=self.stale_running_seconds or 120
        )
        ...
        for job in session.exec(
            ...
            try:
                ...
                logger.exception(
                    "strict FIFO stale job recovery failed",
                    extra={"job_id": job.id},
                )
                continue
...
def _handle_web_memory_embedding_backfill(
    ...
    from .web_api.web_memory import (
        backfill_memory_fact_embeddings,
    )
...
def _handle_web_triplet_extract(
    ...
    from .web_ai.knowledge_jobs import (
        handle_triplet_extract,
    )

    return handle_triplet_extract(
        session,
        payload,
    )
...
def _handle_web_hierarchy_build(
    ...
    from .web_ai.knowledge_jobs import (
        handle_hierarchy_build,
    )

    return handle_hierarchy_build(
        session,
        payload,
    )
