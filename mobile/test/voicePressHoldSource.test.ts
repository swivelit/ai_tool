...
describe("chat voice press-and-hold source", () => {
  ...
  it("exposes configurable hands-free settings without overclaiming wake-word enrollment", () => {
    ...
    expect(customiseSource).toContain('from "@/lib/handsFreeRecognizer"');
    expect(customiseSource).toContain('handsFreeRecognizer.start("customise"');
    expect(customiseSource).not.toContain("ExpoSpeechRecognitionModule");
    expect(modalSource).toContain('from "@/lib/handsFreeRecognizer"');
    expect(modalSource).toContain('handsFreeRecognizer.start("modal"');
    expect(modalSource).not.toContain("ExpoSpeechRecognitionModule");
    expect(modalSource).toContain("hands-free, and wake phrase");
    expect(setupSource).toContain("hands-free wake detection");
    expect(setupSource).toContain("OpenWakeWord model");
