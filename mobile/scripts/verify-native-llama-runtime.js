...
const androidRuntimeFile = path.join(androidCppRoot, 'jai_llama_runtime.cpp');
...
function verifyRequiredFiles() {
  ...
  requireFile(androidRuntimeFile, 'Android JNI llama.cpp runtime source');
...
function verifyAndroidRuntimeSafety() {
  const runtime = read(androidRuntimeFile);
  const unsafeGeneratedStringReturn = 'NewString' + 'UTF(generated.c_str())';
  if (runtime.includes(unsafeGeneratedStringReturn)) {
    fail(
      'Android native runtime uses unsafe Modified UTF-8 conversion for generated model output',
      `File: ${path.relative(repoRoot, androidRuntimeFile)}\nUse Java String(byte[], StandardCharsets.UTF_8) for generated bytes instead.`,
    );
  }
  requireContains(
    androidRuntimeFile,
    ...
    'Android native runtime decodes generated output with StandardCharsets.UTF_8',
  );
  requireContains(
    androidRuntimeFile,
    runtime,
    'NewByteArray',
    'Android native runtime marshals generated output through raw byte arrays',
  );
  requireContains(
    androidRuntimeFile,
    runtime,
    '([BLjava/nio/charset/Charset;)V',
    'Android native runtime calls the String(byte[], Charset) constructor',
  );
  if (/kMaxStrongCachedModels\s*=\s*2\b/.test(runtime)) {
    fail(
      'Android native runtime strongly caches too many GGUF models by default',
      `File: ${path.relative(repoRoot, androidRuntimeFile)}\nUse a safe mobile default of one strongly cached model.`,
    );
  }
  requirePattern(
    androidRuntimeFile,
    runtime,
    /kMaxStrongCachedModels\s*=\s*1\b/,
    'Android native runtime defaults to one strongly cached model',
  );
}
