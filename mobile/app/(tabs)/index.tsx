...
export default function Home() {
    ...
    const filteredHistory = useMemo(() => {
        ...
        return conversationGroups.filter((group) => {
            const blob = group.items
                .map((item) =>
                    `${item.title || ""} ${item.details || ""} ${item.raw_text || ""} ${item.intent || ""
                    } ${item.category || ""}`
                )
                .join(" ")
                .toLowerCase();
    ...
    async function playAgentReply(text: string) {
        if (!text) return;
        try {
            const data = await apiPost<{ audio_base64?: string }>("/api/tts", { text });
    ...
    async function analyzeText() {
        ...
        try {
            ...
            if (nextItem.details) {
                playAgentReply(nextItem.details);
            }
