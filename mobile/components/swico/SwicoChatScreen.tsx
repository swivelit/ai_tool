...
export default function SwicoChatScreen() {
  ...
  const send = useCallback(async (override?: string, options: { continueId?: string; editId?: string; regenerateId?: string; inputMode?: InputMode; voiceTurnId?: string | null; requestId?: string } = {}) => {
    ...
    if (!hasSendableContent(text, selectedAttachments, repositoryReady)) return;
  ...
  const latestEditableId = useMemo(() => latestEditableUserId(messages, streaming), [messages, streaming]);
  const sendable = hasSendableContent(draft, attachments, Boolean(repositoryId && repositoryMeta && repositoryUsable({ ...repositoryMeta, owner_uid: repositoryOwnerUid || "", thread_id: repositoryThreadId }, user?.uid || "", activeThread)));
  ...
  return (
    <Screen safeArea={false} glow={false}>
      ...
      <SettingsModal visible={settingsModal} onClose={() => setSettingsModal(false)} user={user} bootstrap={bootstrap} onBootstrap={setBootstrap} offline={offline} onBilling={() => { if (!offline) { setBillingBucket("chat"); setBilling(true); } }} onLegal={setLegalPage} />
...
function SettingsModal({ visible, onClose, user, bootstrap, onBootstrap, offline, onBilling, onLegal }: { visible: boolean; onClose: () => void; user: NonNullable<ReturnType<typeof useAuth>["user"]>; bootstrap: Bootstrap; onBootstrap: React.Dispatch<React.SetStateAction<Bootstrap | null>>; offline: boolean; onBilling: () => void; onLegal: (page: string) => void }) {
  const { palette: t, themePreference, setThemePreference } = useAppTheme();
  const styles = useMemo(() => createStyles(t), [t]);
