...
export default function SwicoChatScreen() {
  ...
  const [voiceReplyStates, setVoiceReplyStates] = useState<Record<string, "idle" | "generating" | "playing" | "paused" | "error">>({});
  ...
  const send = useCallback(async (override?: string, options: { continueId?: string; editId?: string; regenerateId?: string; inputMode?: InputMode; voiceTurnId?: string | null; requestId?: string } = {}) => {
    ...
    const payload = {
      ...
      attachment_ids: selectedAttachments.map(item => item.id), ...(repositoryId && repositoryMeta && repositoryUsable({ ...repositoryMeta, owner_uid: repositoryOwnerUid || "", thread_id: repositoryThreadId }, user.uid, activeThread) ? { repository_id: repositoryId } : {}),
    ...
    try {
      ...
      if ((caught as { name?: string }).name === "AbortError" && stopConfirmedRef.current) {
        ...
        if (apiError.status === 402) { setBillingBucket(apiError.code === "voice_credit_exhausted" ? "voice" : "chat"); setBilling(true); }
        if (repositoryDetachCode(apiError.code || "")) { setRepositoryId(undefined); setRepositoryMeta(null); setRepositoryThreadId(null); setError("The active code repository is no longer attached. Upload it again to continue using repository context."); }
  ...
  const playVoice = useCallback(async (message: Message) => {
    ...
    try {
      ...
      if (soundRef.current && currentState === "playing") {
        await soundRef.current.pauseAsync();
        setVoiceReplyStates(value => ({ ...value, [message.id]: "paused" }));
        return;
      }
      if (soundRef.current && currentState === "paused") {
      ...
      loaded.sound.setOnPlaybackStatusUpdate(status => {
        if (status.isLoaded && status.didJustFinish) {
          ...
          soundRef.current = null;
          setVoiceReplyStates(value => ({ ...value, [message.id]: "idle" }));
        }
      });
    } catch (caught) {
  ...
  }, [activeThread, bootstrap, repositoryId, user]);
  ...
  return (
    <Screen safeArea={false} glow={false}>
      <KeyboardAvoidingView style={styles.fill} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        {logicalMessages.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>How can Swico help?</Text><Text style={styles.emptyText}>Ask a question, upload a document, or continue a conversation from the web.</Text><View style={styles.suggestions}>{["Explain something clearly", "Help me plan a project", "Summarize a document"].map(suggestion => <Pressable key={suggestion} onPress={() => void send(suggestion)} style={styles.suggestion}><Text style={styles.suggestionText}>{suggestion}</Text><Ionicons name="arrow-up-outline" size={15} color={t.muted} /></Pressable>)}</View></View> : <><FlatList ref={listRef} data={logicalMessages} keyExtractor={item => item.id} contentContainerStyle={styles.messages} onScroll={event => { const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent; const next = contentSize.height - (contentOffset.y + layoutMeasurement.height) < 48; pinnedToBottomRef.current = next; setPinnedToBottom(next); }} scrollEventThrottle={100} renderItem={({ item }) => <MessageRow message={item} highlighted={item.id === highlightMessageId} retryTick={retryTick} onRetry={() => retryMessage(item)} onCopy={() => void Clipboard.setStringAsync(item.content)} onShare={() => void Share.share({ message: item.content })} onDownload={() => void downloadResponse(item)} onOpenEditor={() => setEditorMessage(item)} onEditResponse={() => setEditorMessage(item)} onVoice={() => void playVoice(item)} onFeedback={rating => void submitFeedback(item, rating)} onContinue={() => void send(undefined, { continueId: item.id })} onRegenerate={() => { const original = messages.find(candidate => candidate.role === "user" && candidate.request_id === item.request_id); if (original) void send(original.content, { regenerateId: item.id }); }} onEdit={() => { setDraft(item.content); setEditTarget(item.id); }} />} />{!pinnedToBottom ? <Pressable onPress={() => { pinnedToBottomRef.current = true; setPinnedToBottom(true); listRef.current?.scrollToEnd({ animated: true }); }} style={styles.scrollBottom}><Ionicons name="arrow-down" size={18} color={t.text} /></Pressable> : null}</>}
      ...
      <SettingsModal visible={settingsModal} onClose={() => setSettingsModal(false)} user={user} bootstrap={bootstrap} onBootstrap={setBootstrap} onBilling={() => { setBillingBucket("chat"); setBilling(true); }} onLegal={setLegalPage} />
      <SwicoBilling visible={billing} close={() => setBilling(false)} user={user} config={bootstrap.billing} initialBucket={billingBucket} refreshed={async () => { setBootstrap(await getBootstrap(user)); }} />
...
function MessageRow({ message, highlighted, retryTick: _retryTick, onRetry, onCopy, onShare, onDownload, onOpenEditor, onEditResponse, onVoice, onFeedback, onContinue, onRegenerate, onEdit }: { message: Message; highlighted?: boolean; retryTick?: number; onRetry: () => void; onCopy: () => void; onShare: () => void; onDownload: () => void; onOpenEditor: () => void; onEditResponse: () => void; onVoice: () => void; onFeedback: (rating: "up" | "down") => void; onContinue: () => void; onRegenerate: () => void; onEdit: () => void }) {
...
function Drawer({ visible, onClose, threads, hasMore, onLoadMore, active, onSelect, onSelectSearch, onNew, onActions, archived, setArchived, search, setSearch, results, wallet, onBilling, onSettings, onTheme, onLegal, onSignOut }: { visible: boolean; onClose: () => void; threads: Thread[]; hasMore: boolean; onLoadMore: () => void; active: string | null; onSelect: (id: string) => void; onSelectSearch: (id: string, messageId: string | null) => void; onNew: () => void; onActions: (thread: Thread) => void; archived: boolean; setArchived: (value: boolean) => void; search: string; setSearch: (value: string) => void; results: SearchResult[]; wallet: Bootstrap["wallet"]; onBilling: () => void; onSettings: () => void; onTheme: () => void; onLegal: (page: string) => void; onSignOut: () => void }) {
  const { palette: t } = useAppTheme();
  const styles = useMemo(() => createStyles(t), [t]);
...
function SettingsModal({ visible, onClose, user, bootstrap, onBootstrap, onBilling, onLegal }: { visible: boolean; onClose: () => void; user: NonNullable<ReturnType<typeof useAuth>["user"]>; bootstrap: Bootstrap; onBootstrap: React.Dispatch<React.SetStateAction<Bootstrap | null>>; onBilling: () => void; onLegal: (page: string) => void }) {
  const { palette: t, themePreference, setThemePreference } = useAppTheme();
  const styles = useMemo(() => createStyles(t), [t]);
  ...
  useEffect(() => {
    ...
    void Promise.all([getProfileSettings(user), getMemorySettings(user), getUsage(user), getUsageSettings(user)]).then(([nextProfile, memory, nextUsage, nextUsageSettings]) => {
      ...
      setUsageSettings(nextUsageSettings); setUnlimited(nextUsageSettings.hard_limit_micros === null); setWarningPercent(String(nextUsageSettings.warning_threshold_percent)); setNotifyAtThreshold(nextUsageSettings.notify_at_threshold);
    }).catch(() => undefined);
  ...
  }, [monthlyTokens, notifyAtThreshold, unlimited, user, warningPercent]);
