...
export function SwicoBilling({ visible, close, user, config, initialBucket = "chat", refreshed, offline = false }: Props) {
  ...
  return <Modal testID="swico-billing-modal" visible={visible} transparent animationType="slide" onRequestClose={() => { if (!busy) close(); }}>
    <View style={styles.scrim}><View style={styles.modal}>
      <View style={styles.heading}><View><Text style={styles.title}>Top up</Text><Text style={styles.subtitle}>Secure prepaid Token Credits</Text></View><Pressable testID="swico-billing-close" disabled={busy} onPress={close} accessibilityLabel="Close top up"><Ionicons name="close" size={23} color={t.text} /></Pressable></View>
