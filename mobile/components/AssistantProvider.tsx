...
const GOOGLE_CLEANUP_TIMEOUT_MS = 2500;
...
async function cleanupGoogleSession(params: {
  user: User | null;
  ...
  if (!hasProvider(params.user, "google.com")) {
    return;
  }

  if (params.revokeAccess) {
    try {
      await withTimeout(GoogleSignin.revokeAccess(), GOOGLE_CLEANUP_TIMEOUT_MS);
    } catch (error) {
      console.warn("[auth] Google revokeAccess failed:", error);
    }
  }

  try {
    await withTimeout(GoogleSignin.signOut(), GOOGLE_CLEANUP_TIMEOUT_MS);
  } catch (error) {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function signOutUser() {
    ...
    setUser(null);
    await clearProfile();
    await cleanupGoogleSession({ user: currentUser, revokeAccess: false });
  }
  ...
  async function deleteCurrentAccount(backendUserId?: number) {
    ...
    setUser(null);
    await clearProfile();
    await cleanupGoogleSession({ user: currentUser, revokeAccess: true });
  }
