...
export function CliAuthorizePage() {
  ...
  return <main className="login-page"><section className="auth-card" aria-labelledby="cli-authorize-title"><div className="auth-brand"><span className="brand-mark" aria-hidden="true">S</span><span>Swico</span></div><h1 id="cli-authorize-title">Authorize Swico CLI</h1>{device && <><p><strong>{device.device_description}</strong> is requesting access to your Swico account.</p><p>Code: <strong>{device.user_code}</strong></p><p>Requested access: {device.scopes.join(', ') || 'chat'}. Your existing account tier and Chat billing rules apply.</p>{device.status === 'pending' && <div className="auth-links"><button className="primary" disabled={busy} onClick={() => void decide(true)}>Approve terminal</button><button disabled={busy} onClick={() => void decide(false)}>Deny</button></div>}</>}{status && <p role="status" className="auth-status">{status}</p>}</section></main>
}
