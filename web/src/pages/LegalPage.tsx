...
export function LegalPage() {
  ...
  const publicationStatus = legalContent.publication.publicationStatus
  const published = publicationStatus === 'owner_approved' || publicationStatus === 'approved_by_counsel'
  return <main className="legal-page"><Link to="/"><ArrowLeft size={16} /> Back to Swico</Link><div className="legal-heading"><span>Swico</span><h1>{content.title}</h1><p>{content.summary}</p><small>Version {content.version} · Effective date: {content.effectiveDate || 'Not published'}</small></div>
    {!published && <aside className="legal-warning"><strong>Policy text is not published</strong><p>This page is a publication framework. Complete policy text and accountable publication metadata must be supplied before release.</p></aside>}
    <div className="legal-sections">{content.sections.map((section, index) => <section key={section.heading}><span>{String(index + 1).padStart(2, '0')}</span><div><h2>{section.heading}</h2><p>{section.body || 'Publication text has not been supplied.'}</p></div></section>)}</div>
  </main>
}
