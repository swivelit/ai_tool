...
function subscriptionPlanLabel(code: string | null | undefined) {
  return ({ '1m': '1 month', '6m': '6 months', '1y': '1 year' }[code ?? ''] ?? 'Subscription')
}
...
function UsageBars({ usage }: { usage: UsageSummary }) {
  const tiers = (['free', 'lite', 'standard', 'pro'] as const).map(id => {
    ...
    return {
      id, label: item.label, tokenEstimate: item.token_estimate,
      ...
      detail: `${item.request_count.toLocaleString()} requests · ${item.input_tokens.toLocaleString()} input · ${item.cached_input_tokens.toLocaleString()} cached · ${item.output_tokens.toLocaleString()} output · ${item.total_tokens.toLocaleString()} actual tokens`, unit: 'tokens' as const,
  ...
  const items = [...tiers, {
    id: 'voice', label: voice.label, tokenEstimate: voice.token_estimate,
    ...
    unit: 'token equivalent' as const,
  ...
  return <div className="usage-breakdowns" aria-label="Usage by category">
    {items.map(item => {
      ...
      return <article key={item.id} className="usage-breakdown">
        <div><strong>{item.label}</strong><span>{tokenEstimateLabel(item.tokenEstimate, item.unit)}</span></div>
        <div className="usage-progress" role="progressbar" aria-label={`${item.label} credit utilization`}
          aria-valuemin={0} aria-valuemax={100} aria-valuenow={usage.billing_exempt ? undefined : value} aria-valuetext={usage.billing_exempt ? 'Unlimited' : `${value}% utilization; ${tokenEstimateLabel(item.tokenEstimate, item.unit)} used`}>
          <span style={{ width: `${value}%` }} />
        </div>
        <small>{usage.billing_exempt ? 'Unlimited' : `${value}% · ${tokenEstimateLabel(item.tokenEstimate, item.unit)} used · ${item.basis === 'monthly_hard_limit' ? 'monthly limit' : 'available balance plus period debit'}`}</small>
        <small>{item.detail}{item.tokenEstimate?.estimated_blended_tokens === 0 ? ' · No usage yet' : ''}</small>
      </article>
    })}
  </div>
}
...
setCustomColors?: (colors: CustomColors) => void; close: () => void;
  ...
  const tokenRange = useMemo(() => tokenEstimateLabel(estimate), [estimate])
  ...
  <label>
    ...
    <small id="interface-language-help">
      Reply languages are available in Profile. The settings interface currently supports English.
        ...
        <label htmlFor="profile-reply-language">Reply language<select id="profile-reply-language" value={profile.reply_language} onChange={event => setProfile({ ...profile, reply_language: event.target.value as ReplyLanguage })}>{WEB_REPLY_LANGUAGES.map(language => <option key={language} value={language}>{REPLY_LANGUAGE_NATIVE_LABELS[language]}</option>)}</select></label>
          ...
          {loaded && section === 'usage' && <section aria-labelledby="usage-settings"><h3 id="usage-settings">Token credits</h3>
            {subscriptionData?.enabled && <div className="subscription-settings-cards"><h4>Subscriptions</h4>{(['chat', 'voice'] as const).map(bucket => { const item: SubscriptionBucketSummary | null | undefined = subscriptionData[bucket]; const unit = bucket === 'voice' ? 'token equivalent' as const : 'tokens' as const; return <article key={bucket}><div><strong>{bucket === 'chat' ? 'Chat' : 'Voice'} subscription</strong><span>{item?.active ? `${item.source === 'referral_reward' ? 'Referral reward' : 'Plan'} · ${subscriptionPlanLabel(item.plan)}` : 'Inactive'}</span></div>{item?.active && <><div className="usage-progress" role="progressbar" aria-label={`${bucket} subscription allowance used`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(item.progress_percent)} aria-valuetext={`${tokenEstimateLabel(item.consumed_token_estimate, unit)} used; ${tokenEstimateLabel(item.remaining_token_estimate, unit)} remaining`}><span style={{ width: `${Math.min(100, item.progress_percent)}%` }} /></div><small>Allowance {tokenEstimateLabel(item.allowance_token_estimate, unit)} · Used {tokenEstimateLabel(item.consumed_token_estimate, unit)} · Reserved {tokenEstimateLabel(item.reserved_token_estimate, unit)} · Remaining {tokenEstimateLabel(item.remaining_token_estimate, unit)}</small><small>Next reset {item.next_reset_at ? new Date(item.next_reset_at).toLocaleString() : '—'} · Expires {item.expires_at ? new Date(item.expires_at).toLocaleString() : '—'}</small><small>No rollover. {bucket === 'voice' ? (item.voice_estimate?.note ?? 'Voice usage can include speech processing and AI response generation.') : 'Chat allowance is shown with your tier-aware estimate.'}</small>{item.queued_entitlements.length > 0 && <small>Queued: {item.queued_entitlements.map(entry => subscriptionPlanLabel(entry.plan)).join(', ')}</small>}</>}{item && <label className="check-row"><input type="checkbox" checked={item.payg_fallback_enabled} onChange={event => void toggleFallback(bucket, event.target.checked)} />Allow pay-as-you-go fallback when this window cannot cover the full request</label>}</article>})}</div>}
            <div className="usage-cards"><article><span>Chat tokens available</span><strong>{billingExempt ? 'Unlimited' : tokenEstimateLabel(loaded.usage.chat_available_token_estimate)}</strong></article><article><span>Voice estimated token equivalent available</span><strong>{billingExempt ? 'Unlimited' : tokenEstimateLabel(loaded.usage.voice_available_token_estimate, 'token equivalent')}</strong></article>{!billingExempt && <article><span>{loaded.usage.tier_label} estimated token range</span><strong>{tokenRange}</strong></article>}</div>
            {!billingExempt && loaded.usage.tester_credit?.active && <article className="usage-card"><span>Weekly tester credit</span><strong>{loaded.usage.tester_credit.available_micros?.toLocaleString() ?? 'Unavailable'} micros remaining</strong><small>Resets {loaded.usage.tester_credit.period_end ? new Date(loaded.usage.tester_credit.period_end).toLocaleString() : '—'} · Chat only</small></article>}
