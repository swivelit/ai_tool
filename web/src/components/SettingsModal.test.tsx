...
const usage = { period:'current_month' as const, tier:'lite' as const, tier_label:'Swico Lite', timezone:'Asia/Kolkata', period_start:'2026-07-31T18:30:00Z', period_end:'2026-08-31T18:30:00Z', next_reset_at:'2026-08-31T18:30:00Z', request_count:2, input_tokens:1200, cached_input_tokens:300, output_tokens:400, total_tokens:1600, actual_usage_count:1, estimated_usage_count:1, debited_micros:250_000, debited_ai_credits:'0.250000', available_micros:5_000_000, available_ai_credits:'5.000000', daily:[], monthly_hard_limit_micros:null, by_tier:{ free:emptyTier('Swico Free'), lite:{ ...emptyTier('Swico Lite'), request_count:2, input_tokens:1200, cached_input_tokens:300, output_tokens:400, total_tokens:1600, debited_micros:250_000, debited_token_credits:'0.250000', period_debit_percentage:100 }, standard:emptyTier('Swico'), pro:emptyTier('Swico Pro') }, voice:{ label:'Voice' as const, stt_request_count:0, tts_request_count:0, llm_request_count:1, llm_input_tokens:80, llm_cached_input_tokens:15, llm_output_tokens:20, llm_total_tokens:100, total_audio_seconds:0, total_tts_characters:0, request_count:1, debited_micros:7_000, debited_voice_credits:'0.007000', period_debit_percentage:2.8, monthly_limit_percentage:0 }, estimated_tokens_remaining:{ tier:'lite' as const, tier_label:'Swico Lite', pricing_as_of:'2026-07-17T00:00:00Z', estimated_blended_tokens:60_000, blended_assumption:'70/30', range_min_tokens:25_000, range_max_tokens:180_000, explanation:'Estimated for Swico Lite. Actual usage depends on message size, response length, and task complexity.' } }
...
it('shows concise tier estimates and this-month token categories', async () => {
  ...
  expect(screen.getByRole('progressbar', { name:'Voice credit utilization' })).toHaveAttribute('aria-valuenow', '2.8')
  expect(screen.getByText(/realtime voice uses voice credits for transcription, AI response generation, and speech/i)).toBeInTheDocument()
  expect(screen.getByText(/1 AI response requests/)).toBeInTheDocument()
