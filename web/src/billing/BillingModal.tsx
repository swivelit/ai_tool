...
function voiceEstimateLabel(estimate: VoiceCreditEstimate | null | undefined) {
  return estimate ? `About ${estimate.estimated_stt_minutes} STT min or ${estimate.estimated_tts_characters.toLocaleString()} TTS characters` : 'Estimate unavailable'
}
    ...
    <section ref={dialogRef} className="billing-modal" role="dialog" aria-modal="true" aria-labelledby="billing-title">
      ...
      {tab === 'history' ? <div className="billing-history" aria-live="polite">
        ...
        {history.map(item => { const presentation = paymentPresentation(item); const itemBucket = item.credit_bucket ?? 'chat'; return <article key={item.id}><header><strong>{presentation.heading}</strong><span className="credit-bucket-label">{itemBucket === 'voice' ? 'Voice' : 'Chat'} credits</span>{presentation.detail && <span>{presentation.detail}</span>}</header><dl>
          ...
          {presentation.showTokensAdded && itemBucket === 'voice' && <div><dt>Voice estimate</dt><dd>{voiceEstimateLabel(item.voice_estimate)}</dd></div>}
      ...
      {selectedAmountPaise !== null && selectedEstimateReady && <div className="package-summary"><strong>Pay {packageRupees(selectedAmountPaise)} for {bucket === 'chat' ? 'Chat' : 'Voice'} credits</strong>{bucket === 'chat' ? <span>Estimated token range <strong>{estimateRange(selectedTokenEstimate)}</strong></span> : <><span>STT-only estimate <strong>{selectedVoiceEstimate?.estimated_stt_minutes} minutes</strong></span><span>TTS-only estimate <strong>{selectedVoiceEstimate?.estimated_tts_characters.toLocaleString()} characters</strong></span><small>{selectedVoiceEstimate?.assumption}</small></>}</div>}
