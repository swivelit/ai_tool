...
export type CloudArtifact = { id: string; job_id: string; attempt: number; kind: string; content_type: string; sha256: string; size_bytes: number; created_at: string }
...
export async function listCloudArtifacts(user: User, id: string, signal?: AbortSignal): Promise<{ items: CloudArtifact[] }> { return apiJson<{ items: CloudArtifact[] }>(user, `/api/web/cloud/jobs/${encodeURIComponent(id)}/artifacts`, { signal }) }
export async function downloadCloudArtifact(user: User, jobId: string, artifactId: string): Promise<Blob> { const response = await authorizedFetch(user, `/api/web/cloud/jobs/${encodeURIComponent(jobId)}/artifacts/${encodeURIComponent(artifactId)}`); if (!response.ok) throw new ApiError(response.status, await response.json().catch(() => ({}))); return response.blob() }
