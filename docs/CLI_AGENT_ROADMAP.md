...
Commands use the user's permissions inside the local OS sandbox when the
runtime probe succeeds; platform readiness is reported and unsupported hosts
fail closed.
...
Read-only and workspace-write sandbox policies are separate from the
existing approval profile. Network is disabled by default and child
environments are reduced to basic runtime variables. Stage 3 does not claim
an OS sandbox for the API service.
...
The following remain later-stage work: a reviewed Windows sandbox, durable
