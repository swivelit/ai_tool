...
3. In the initial Blueprint form, provide every `sync:false` value. On the API,
   set `CORS_ALLOW_ORIGINS` to the exact staging web HTTPS origin; leave
   `CORS_ALLOW_LOCAL_DEV_ORIGINS` empty unless local Vite -> hosted-backend
   development is explicitly required. If it is required, use only
   `http://localhost:5173,http://127.0.0.1:5173`; provide a
...
### Paid current-information lookup (backend only)

The paid website tiers may use the bounded OpenAI Responses web-search
adapter. Keep the existing `ENABLE_WEB_SEARCH_FOR_FREE=false`; it is the
separate Free/local-only switch. Add these variables only to the existing API
service, never to `swico-web`, `VITE_*`, or a shared client environment:

```dotenv
WEB_LIVE_SEARCH_ENABLED=false
WEB_LIVE_SEARCH_PROVIDER=openai
WEB_LIVE_SEARCH_MODEL=gpt-4.1-mini
WEB_LIVE_SEARCH_TIMEOUT_SECONDS=15
WEB_LIVE_SEARCH_MAX_CALLS_PER_TURN=1
WEB_LIVE_SEARCH_MAX_OUTPUT_TOKENS=1000
```

Set `WEB_LIVE_SEARCH_ENABLED=true` only with the existing backend
`OPENAI_API_KEY`. The adapter validates the provider/model and bounds, requires
completed hosted search output with source citations, and is admitted within
the existing paid request reservation. Roll back by setting
`WEB_LIVE_SEARCH_ENABLED=false`, then redeploying the API service. Verify the
configuration offline with `cd backend && python scripts/web_live_search_check.py --pretty`;
...
Before enabling either flag, deploy `ai_tool` and `swico-web` from the same Git commit and verify that the backend release check reports `blocker_count=0`, matching repository/database Alembic heads, and the required subscription schema revision `b8f2c7d1e4a9` in the current migration ancestry. Newer descendant migrations are valid; production must be migrated to the current repository head. If the backend is updated but the website still shows old billing text, use `swico-web → Manual Deploy → Clear build cache & deploy`, then repeat the frontend verification. This is a deployment check only; it adds no Render resource or variable.
...
Production was last audited at `20260915_weekly_tester_credit`; this checkout
adds the pending `20260917_cli_cloud_artifacts` descendant for the cloud control
plane. Apply it only through the normal pre-deploy migration owner before any
cloud-pilot check; public Chat does not require cloud readiness.
No migration is
...
The previously deployed single Alembic head `20260915_weekly_tester_credit` (which descends from
...
production baseline head was `20260915_weekly_tester_credit`; the current checkout adds `20260916_cli_cloud_jobs` and the bounded `20260917_cli_cloud_artifacts` descendant. Historical revision `b4e8c1d6a2f9` adds the
...
2. Run `cd backend && python -m alembic -c alembic.ini upgrade head` as the pre-deploy step; confirm the repository-derived head `20260917_cli_cloud_artifacts` for this checkout.
...
3. Use the API service pre-deploy command:
   ...
   `20260917_cli_cloud_artifacts` (which descends from
   `20260916_cli_cloud_jobs`).
...
## Swico Cloud runner (do not enable yet)
...
traffic away from the runner; never fall back to API-local execution.
