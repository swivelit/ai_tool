...
3. Run one small paid Chat request with the installed CLI. Confirm the server
   returns a normal response and record the usage value afterward.
...
5. In the website, revoke the CLI session while the local credential record is
   still present. Call `swico whoami` and one paid Chat request; both must be
   rejected by the server. Confirm the retained local record was not treated as
   authorization.
