...
1. In a clean disposable desktop shell, confirm the exact installed artifact
   with `swico --version --json`; record version, revision, dirty state, and
   executable path. Do not proceed if it is not the approved RC artifact.
2. Run `swico login --tier lite` (or the separately approved `standard`/`pro`
   tier) and complete fresh browser approval. Then run `swico whoami` and
   `swico doctor`; record only account identity, tier, API status, and auth-valid
   status. Do not substitute Swico Free.
3. Run `swico usage --json`, record the website Chat usage/wallet value without
   exposing credentials, then enter the installed interactive CLI and run
   `/usage`; record the
   ...
   a Swico command.
4. Wait for the controlled 900-second access-token expiry in the
